...
/*   Updated: 2025/01/13 15:42:09 by dstinghe         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	alloc_mem(t_philo_data *data)
{
	...
	data->stop_threads = malloc(sizeof(*data->stop_threads)
			* data->philo_count);
	data->lock_threads = malloc(sizeof(*data->lock_threads)
			* data->philo_count);
...
static int	mutex_init_all(t_philo_data *data)
{
	...
	if (mutex_init_arr(data->lock_threads, data->philo_count))
	{
		if (pthread_mutex_destroy(&data->lock_printf))
			printf("%s %s\n", ERRMSG_INTERNAL, ERRMSG_MUTEX_DESTR);
